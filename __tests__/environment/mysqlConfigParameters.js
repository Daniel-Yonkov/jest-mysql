...
it("Check DB config using Host & User only ", async () => {
    ...
    expect(execMethod).toHaveBeenCalledWith(
...
it("Check DB config using Port ", async () => {
    ...
    expect(execMethod).toHaveBeenCalledWith(
        "mysql -h 127.0.0.1 -u root -P 3306  < FILE2"
    );
});
...
it("Check DB config using password ", async () => {
    ...
    expect(execMethod).toHaveBeenCalledWith(
        "mysql -h 127.0.0.1 -u root -psuperSecret  < FILE"
    );
});
...
it("Check DB config using database parameter ", async () => {
    ...
    expect(execMethod).toHaveBeenCalledWith(
        "mysql -h 127.0.0.1 -u root jest_mysql_test < FILE"
    );
});
...
it("Check seven part config ", async () => {
    ...
    expect(execMethod).toHaveBeenCalledWith(
        "mysql -h 127.0.0.1 -u root -proot -P 3306 jest_mysql_test < FILE"
    );
});
